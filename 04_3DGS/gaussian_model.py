...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        # Compute mean distance to K nearest neighbors using pure PyTorch
        K = min(50, self.n_points - 1)
        points = points3D_xyz  # (N, 3)

        # Compute pairwise distances (use batched for memory efficiency)
        # For large point clouds, process in chunks
        chunk_size = 1024
        n = points.shape[0]
        mean_dists = torch.zeros(n, 1)

        for start in range(0, n, chunk_size):
            end = min(start + chunk_size, n)
            chunk = points[start:end]  # (C, 3)
            dists = torch.cdist(chunk, points)  # (C, N)
            # K+1 because the closest point is the point itself (distance 0)
            topk_dists, _ = torch.topk(dists, K + 1, dim=1, largest=False)
            mean_dists[start:end] = torch.sqrt(topk_dists[:, 1:]).mean(dim=1, keepdim=True) * 2.0

        mean_dists = mean_dists.clamp(0.2 * torch.median(mean_dists), 3.0 * torch.median(mean_dists))
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Since S is diagonal, S @ S^T = S^2 (squared scales)
