...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Sample 3D points to a specific number (using pure PyTorch FPS)
        if self.points3D_xyz.shape[0] > maximum_pts_num:
            indices = self._farthest_point_sample(self.points3D_xyz, maximum_pts_num)
    ...
    def _farthest_point_sample(points: torch.Tensor, K: int) -> torch.Tensor:
        """Farthest point sampling (FPS) using pure PyTorch."""
        N = points.shape[0]
        if K >= N:
